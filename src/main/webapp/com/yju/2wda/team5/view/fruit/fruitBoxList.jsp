...
<hr>
<form class="d-flex" method="post" action="./fruitBoxList.be">
...
<div class="link">
<a href="./fruitBoxList.be?currentPageNo=0&keyword=<%=request.getAttribute("keyword")%>">[FIRST]</a>
<%
	if(currentPageNo > 0) {
%>
	<a href="./fruitBoxList.be?currentPageNo=<%=(currentPageNo-1)%>&keyword=<%=request.getAttribute("keyword")%>">[PRE]</a>
<%
	}else{
%>
	[PRE]
...
%>
			<a href="./fruitBoxList.be?currentPageNo=<%=i%>&keyword=<%=request.getAttribute("keyword")%>">[<%=(i+1)%>]</a>
<%
		}
	}
%>
...
%>
	<a href="./fruitBoxList.be?currentPageNo=<%=(currentPageNo+1)%>&keyword=<%=request.getAttribute("keyword")%>">[NXT]</a>
<%
	} else {
%>
	[NXT]
...
%>
<a href="./fruitBoxList.be?currentPageNo=<%=(fpiVO.getPageCnt()-1)%>&keyword=<%=request.getAttribute("keyword")%>">[END]</a>
</div>
</div>
</body>
</html>